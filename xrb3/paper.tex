...
neutron star \citep{bhattacharyya:2007}. {\color{red} The next sentence
is long and it was hard to read on my initial pass over it. Could consider
splitting the parenthetical note off as a separate sentence, and switching
to active voice (write ``and we need hydrodynamic simulations" and replace
``are needed" with ``in order").} 
This spreading is inherently
a multi-dimensional phenomenon, and hydrodynamic simulations that
...
{\color{red} Could split paragraph and start next sentence with
"Here we show...".}
...
early evolution. {\color{red} Very minor: could start previous sentence with
``consequently" or ``therefore" since ``as a result" appears in the previous sentence.}
...
from above. {\color{red} It might be helpful to have the time for each panel either
on the plots or in this caption.}
}
\end{figure}
...
simulation {\color{red} at 40~ms} showing the entire domain from the side.  The
flame structure is clearly seen. {\color{red} The flame structure could probably
be made a little clearer if this only showed the lower $10^4~\mathrm{cm}$ of the
domain like the 2D plots. Could also try further reducing the peak $\bar{A}$ on the
colorbar (e.g. to 5.5 or 5.25) both here and in the 2D plots to bring out the flame
even more. I think the plots are still pretty clear with the current colorbar though.}}
\end{figure*}
