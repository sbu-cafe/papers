...
to to an additional constraint on velocity discussed below:
\begin{eqnarray}
...
the base state).  We also define a base state density, $\rho_0 = \rho_0(r,t)$, that represents the lateral average (see Section \ref{Sec:Spatial}) of $\rho$ and is in hydrostatic equilibrium with $p_0$, i.e., 
...
Mathematically, equations (\ref{eq:momentum})-(\ref{eq:enthalpy}) must still be closed by an EOS.  
This is done by taking the Langrangian derivative of the EOS for pressure as a function of the density, composition, and enthalpy, requiring that the pressure is a prescribed function of altitude and time based on the hydrostatic equilibrium condition.
Details are given in Papers I and II, but to summarize, after substituting the equations for mass and energy into the Lagrangian derivative, the result
...
To summarize, we model equations (\ref{eq:momentum})-(\ref{eq:enthalpy})
subject to (\ref{eq:U divergence}) and the hydrostatic equilibrium condition.
