...
New algorithm... overframework is similar but now more splitting $w_0$ dynamics... eliminates the need to explicitly evolve the base state and compute the average expansion... instead we can just use a predictor-corrector treatment of the base state while retaining the same formal order of accuracy...
...
At the beginning of each time step we have the state,
$(\Ub,\rho X_k,\rho h,\rho_0,p_0)^n$, and $\pi^{n-\myhalf}$.
...
T^n = T(\rho,p_0,X_k)^n.
\end{equation}
Similarly, $\gammaonebar$ and $\beta_0$ are computed using $\rho,p_0$, and $X_k$ (see Appendix A of Paper I and Appendix C of Paper III for details).

The overal flow of the algorithm is a second-order Strang Splitting approach for the splitting of the reaction and advection steps.  Furthermore, we use a predictor-corrector approach within this splitting scheme to achieve second-order accurate treatment of the constraing equation.  To summarize:
\begin{itemize}
\item In {\bf Step 1} we react the thermodynamic variables over $\Delta t/2$.
\item In {\bf Steps 2-4} we advect the thermodynamic variables over $\Delta t$.
\item In {\bf Step 5} we react the thermodynamic variables over $\Delta t/2$.
...
\beta_0^{\nph} \left(S^{\nph} - \frac{1}{\gammaonebar^{n+\myhalf}p_0^{n+1,\star}}\frac{p_0^{n+1,\star}-p_0^n}{\Delta t^n}\right),
\end{equation}
where
\begin{equation}
