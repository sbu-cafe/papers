...
\begin{abstract}
We present MAESTROeX, a next-generation solver for low Mach number astrophysical flows.
Here we summarize implementation details as well new spatiotemporal integration schemes
developed for increased accuracy and a simpler base state treatment.  The code is written
in the AMReX software framework for massively parallel block-structured adaptive mesh
...
show strong and/or weak scaling performance (with OpenMP threads/tiling), comparisons with old MAESTRO?

\section{Test Problems}
white dwarf convection runs at varying resolution
