...
Many astrophysical flows are highly subsonic; 
sound waves carry sufficiently little energy that they do not significantly affect the convective dynamics of the system.
...
In a low Mach number approach, sound waves are eliminated from the governing equations by requiring that the
thermodynamic variables evolve subject to a specified ambient pressure that varies only as a function of altitude and time.
...
Call {\bf React State}$[\rho^n, (\rho h)^n, X_k^n, T^n, p_0^n] \rightarrow [\rho^{(1)},(\rho h)^{(1)},X_k^{(1)},T^{(1)},(\rho \omegadot_k)^{(1)},(\rho \Hnuc)^{(1)}]$.
...
Call {\bf React State}$[ \rho^{(2),\star},(\rho h)^{(2),\star}, X_k^{(2),\star}, T^{(2),\star}, p_0^{n+1,\star}]$\\
...
\beta_0^{\nph} \left(S^{\nph} - \frac{1}{\overline{\Gamma}_1^{n+\myhalf}p_0^{n+1,\star}}\frac{p_0^{n+1,\star}-p_0^n}{\Delta t^n} + \chi^{n+\myhalf}\right),
\end{equation}
\begin{equation}
...
Call {\bf React State}$[\rho^{(2)},(\rho h)^{(2)}, X_k^{(2)},T^{(2)}, p_0^{n+1}] \rightarrow [\rho^{n+1}, (\rho h)^{n+1}, X_k^{n+1}, T^{n+1}, (\rho \omegadot_k)^{(2)}, (\rho \Hnuc)^{(2)} ].$
