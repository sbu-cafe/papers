...
This new solver leverages the algorithmic capabilities of the previous MAESTRO code
while taking advantage of the next-generation AMReX software framework block-structured
adaptive mesh refinement applications.
Using our previous studies on white dwarf convection as a benchmark, we characterize
the performance of our new code.
We have also developed new temporal integration schemes that have a much simpler treatment
of the base state, while allowing for more straightforward extensions to state-of-the
art, high-order multiphysics coupling schemes.
Also, we present an improved spatial coupling scheme between the hydrostatic base
state and the Cartesian grid full-state.
\end{abstract}
...
In a low Mach number approach, sound waves are eliminated from the governing equations, and the resulting model can be numerically integrated with much larger time steps than a compressible model.

Previously, we have developed the low Mach number astrophysical solver, MAESTRO.  MAESTRO is unique in two ways.  First, the low Mach number model is specifically derived for astrophysical settings.  Second, MAESTRO incorporates the use of a stratified background (or base) state density and pressure held in hydrostatic equilibrium, and also vary as a function of altitude and time.  MAESTRO is suitable for for spherical stars, as well as planar simulations of local dynamics.

by requiring that the thermodynamic variables evolve subject to a specified ambient pressure that varies only as a function of altitude and time.
In the low Mach equations, the equation of state is recast as a divergence constraint on velocity, requiring an
elliptic solve for pressure in the spirit of projection methods for incompressible flow.

(overview of MAESTRO numerics) finite volume, AMR. include definition of planar and spherical.
...
We would like to revamp our previous MAESTRO code by moving to the AMReX \cite{AMReX} framework
...
Previously we adopted an approach where we split the velocity into a base state component, $w_0$, and a local velocity $\Ubt$, so that
\begin{equation}
...
\MarginPar{talk about cutoff densities}
...
The initialization step computes initial values for $\pi^{-\myhalf}$, , $\eta_{\rho}^{-\myhalf}$, 
$\psi^{-\myhalf}$, $S^0$, and $S^1$ for use in time stepping routine.  
We pass through {\bf Step 1} -- {\bf Step 12}, but only keep these values.
...
\begin{equation}
\nabla \cdot \left(\beta_0^n \uadvone\right) = \beta_0^n \left(S^{\nph,\star} - \frac{1}{\overline{\Gamma}_1^np_0^n}\psi^{n-\myhalf} \right),
\end{equation}
\begin{equation}
...
\phantom{ }\hfill $\rightarrow [ (\rho X_k)^{n+1,\star}, (\rho h)^{n+1,\star}, (\rho \omegadot_k)^{(2),\star}, (\rho \Hnuc)^{(2),\star} ].$
...
\begin{equation}
S^{n+1,\star} =  -\sigma  \sum_k  \xi_k  (\omegadot_k)^{(2),\star}  + \frac{1}{\rho^{n+1,\star} p_\rho} \sum_k p_{X_k}  ({\omegadot}_k)^{(2),\star} + \sigma \Hnuc^{(2),\star},
\end{equation}
...
the updated value $S^{\nph}$ rather than $S^{\nph,\star}$.
Also, the temporal changes of $p_0$ are now computed directly instead of 
approximated using $\psi$.
We note that $\uadvtwo$ satisfies the divergence constraint
\begin{equation}
\nabla \cdot \left(\beta_0^{\nph} \uadvtwo\right) =
\beta_0^{\nph} \left(S^{\nph} - \frac{1}{\overline{\Gamma}_1^{n+\myhalf}p_0^{n+1,\star}}\frac{p_0^{n+1,\star}-p_0^n}{\Delta t^n}\right),
\end{equation}
\begin{equation}
...
Call {\bf React State}$[(\rho X_k)^{(2)},(\rho h)^{(2)}, p_0^{n+1}] \rightarrow [(\rho X_k)^{n+1}, (\rho h)^{n+1}, (\rho \omegadot_k)^{(2)}, (\rho \Hnuc)^{(2)}].$
