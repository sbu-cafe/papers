...
(description of base state mapping in planar, and origianl spherical)


\subsection{Exact Base State Mapping} \label{sec:exactbasestate}
...
the acceleration due to the buoyancy force (commonly used when the velocity is approximately zero at the start of some simulations) 
...
\item Update $\rho_0$ by setting $\rho_0^{n+1,\star} =$ {\bf Average}$(\rho^{(2),\star})$.

\item Update $p_0$ by calling
{\bf Enforce HSE}$[\rho_0^{n+1,\star}] \rightarrow [p_0^{n+1,\star}]$.
