...
by the number of contributing cells to obtain the arithmetic average.  However we do not map this onto a uniformly spaced
...
We now describe the new temporal integration scheme, which we note can be used for the original base state mapping (with uniform base state grid spacing) as well as the new irregularly spaced base state mapping.
...
Our new temporal integration scheme uses full velocities to evolve advection and reactions instead, and only uses the above splitting to satisfy the velocity divergence constraints due to boundary considerations. 
This produces a cleaner scheme than the one from Paper V since we use the velocity directly rather than more complex terms involving the perturbational velocity.
