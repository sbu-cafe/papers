...
Specifically, for every Cartesian cell, there exists an integer $m$ such that the radius to the center of the star is given by 
\begin{equation}\label{eqn:radii}
\hat{r}_m=\Delta x\sqrt{0.75+2m}.
\end{equation}
...
The idea is to follow the same method performed on planar geometries by simply computing an arithmetic average of cells that share the same radial distance from the center of the star. 
Figure \ref{Fig:NewBaseGrid} illustrates the relationship between the Cartesian grid state and the one-dimensional array base state with this new mapping technique. Again, these radial distances, and hence the cell-center locations in the new base state array, can be predetermined using equation (\ref{eqn:radii}). Also note that $\Delta r$ decreases as the base state moves further from the center of the star, which results in far more total cells in the irregularly-spaced array than the previous uniformly-spaced array. 
A potential benefit to eliminating the mapping error is to consider a spherical star in hydrostatic equlibrium at rest.  In the absense of reactions, the star should remain at rest.
...
A direct mapping between the radial array cell centers (red squares) and the Cartesian grid cell centers (blue crosses) 
...
\item {\bf Enforce HSE}$[\rho_0]\rightarrow[p_0]$ computes the base state pressure, $p_0$, from a base state density, $\rho_0$ by integrating the hydrostatic equilibrium condition in one dimension. This follows equation (A10) in Paper V with the exception of computing a non-constant $\Delta r_{j+1/2} = r_{j+1} - r_j$ for cell-edge $j+1/2$ if using irregularly-spaced base states.  The base state pressure remains equal to a constant value from the location of a prescribed cutoff density outward for the entire simulation.
