...
We perform weak scaling tests for simulations of convection preceding ignition in a spherical, full-star sub-Chandrasekhar mass white dwarf. \MarginPar{is this really the sub-Chandra stuff? or was it wdconvect, which is Chandra?}
...
\MarginPar{why is the new temporal with constant dr faster?}
...
We continue to use the full-star white dwarf problem described in Section \ref{sec:whitedwarf} to test adaptive simulations on spherical geometry. The adaptive grid is refined twice by tagging the density at $\rho > 10^5$ g cm$^{-3}$ on the first level and $\rho > 10^8$ g cm$^{-3}$ on the second level. 
\MarginPar{possibly mention that we found the code encounters numerical difficulties when the tagging values are too close to each other in subsequent levels of refinement}
