...
over all time nodes for each iteration.  Denoting the time nodes with
...
We use a finite-volume formalism, so $\avg{\Uc}$ is the spatial
average of the conserved state in a zone, $\avg{\Advs{\Uc}}^m$ is the
