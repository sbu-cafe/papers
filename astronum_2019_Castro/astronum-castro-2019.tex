...
level runs are running now, but the lack of AMR makes the very
expensive.
...
estimates show that we can expect a speed-up of atleast $5\times$ when
comparing the NERSC Cori Haswell nodes (using CPUs) to the OLCF Summit
nodes (using GPUs).  Work underway with further optimize the GPU code,
