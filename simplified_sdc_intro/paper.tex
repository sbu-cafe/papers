...
$X_k$, and auxiliary composition variables, $\alpha_l$.  We also carry
the specific internal energy, $e$, as part of a dual energy
formulation (see \citealt{bryan:1995,wdmergerI}). The mass fractions
...
\begin{equation}
  \Sc(\Uc) = \Hb(\Uc) + \Rb(\Uc).
\end{equation}
The hydrodynamic sources include the ``pdV'' work for internal energy
as well as gravity
\begin{equation}
  \Hb(\Ub) = \left ( \begin{array}{c}
    0 \\
    0 \\
    0 \\
    \rho g \\
    \rho u g \\
    -p \partial u/\partial x \end{array} \right )
\end{equation}
 The reactive sources take the form:
\begin{equation}
  \label{eq:react_source}
...
\MarginPar{Formally both $t^n$ or $t^{n+1/2}$ sources give you 2nd-order since they are CTU predictor source terms.  I've never found this type of iteration for this term to amount to any benefit}
      ...
      {\color{red} We enforce that the species interface states remain in $[0, 1]$ after adding the reactive source.}
    ...
    new state with advection and the old-time source term applied for the full $\Delta t$ as: \MarginPar{little more complicated than this---we don't include the pdV source here}
    \begin{equation}
  ...
  \item {\em Update the System Using a Method of Lines Integration}
    ...
    contributions in time from advection.
...
This test problem was first shown in \cite{ABNZ:III}.
...
\end{equation}
For interfacing with the reaction network, we use a simpler set of variables,
\begin{equation}
{\bf w} = \left ( \begin{array}{c} X_\alpha \\  X_\beta \\ e \end{array} \right )
\end{equation}
Note: this intermediate state differs from what we used in \citet{castro_sdc} but agrees with that from \cite{strang_rnaas} in that
...
\frac{\partial {\bf w}}{\partial \Uc^\prime} = \left (
  \begin{array}{ccc}
   \frac{1}{\rho} & 0 & 0 \\
   0 & \frac{1}{\rho} & 0 \\
    -\frac{e_{X_\alpha}}{\rho} & -\frac{e_{X_\beta}}{\rho} & \frac{1}{\rho} \\
   \end{array}\right)
