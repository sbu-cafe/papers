...
approach to spectral deferred corrections coupling of explicit
hydrodynamics and stiff reaction sources and apply it to some example
problems.  This is all done in the framework of the
...
reaction networks and demonstrated the method of a variety of test
problems.  The approach her is considerably simpler, reusing the main
...
However, it is restricted to second-order accuracy overall.  We term this
algorithm the ``simplified SDC method''.
We use the \castro\ hydrodynamics code for all of
...
one-dimensional flow for this description, and the system appears as:
\begin{equation}
...
In the absence of advective terms, our system appears as:
\begin{align}
\odt{\rho} & = 0 \\
...
\odt{(\rho E)} &= \rho \Sdot 
\end{align}
Ww can write the energy equation as:
\begin{equation}
...
X_k)$, if we evolve this system.  This requires an EOS inversion each
...
\end{equation}
We can alternately derive this from an enthalpy equation, which is the correct
formulation when we are at constant pressure, and we get the same temperature
equation but with $c_v$ replaced by $c_p$.
