...
\title{Improved Hydrodynamic/Reaction Coupling for Flows With Nuclear Statistical Equilibrium}
...
\MarginPar{might be worth spelling out exactly what inputs the NSE EOS needs to make things crystal clear}
\begin{equation}
\bar{Z} = \bar{A} Y_e
\end{equation}
  ...
  \begin{itemize}
    ...
    \item For a zone that is in NSE:
      ...
      \item Method II: {\em predictor-corrector.}
        ...
        $Y_e$.  We use these values $\bar{A}_\out$, $(X_k)_\out$, and $(B/A)_\out$ to then complete the update, computing the energy as in Eq.~\ref{eq:nse_energy}.  \MarginPar{we could imagine looping over the EOS to get an updated T and then the table to get the updated $\bar{A}$, to see if that converges?}
...
\begin{equation}
(\rho Y_e)^\star = (\rho Y_e)^n + \Delta t \Advss{\rho Y_e}^{n+1/2} + \Delta t \rho (dY_e/dt)^n
\end{equation}
