...
over a timestep.  An energy limiter takes the form of either:
\begin{equation}
\label{eq:dt:nuce_I}
...
new-time solution.  We use $f = 4$ for the results here.  If these
conditions are violated, then VODE will reject the timestep and retry
with a smaller step.  We also require that the mass fractions are
contained in $[0, 1]$ to a tolerance of $10^{-2}$.
