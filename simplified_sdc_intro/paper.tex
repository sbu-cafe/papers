...
scheme from the original Strang split approach,
making this method easier to add to existing simulation codes.
Overall, compared to the original Strang split approach, this new
approach is more computationally efficient, allows for a larger
time step, and reduces overall integration error.
...
\begin{equation}
  \Sq = \Sqhydro + \Rb(\qb)
\end{equation}\MarginPar{Can we use $\Sc(\qb)$ instead of $\Sq$ and $\Hb(\qb)$ instead of $\Sqhydro$ to keep notation consistent with the conserved variables, since you are already doing that with $\Rb(\qb)$?}
with
\begin{equation}
...
\item {\em Initialization}
  ...
  \item We need an approximation of how much the reactions alone
    ...
    \end{equation}
    \MarginPar{changed to 1-based $k$ iteration for aesthetics}
...
\end{align}
Therefore each of these auxiliary equations obeys an advection
