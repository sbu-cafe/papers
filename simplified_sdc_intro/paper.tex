...
different physical processes can cause the coupling to \replaced{breakdown}{break down, since the reactions do not directly incorporate the effects of the hydrodynamics and vice versa}.
...
\replaced{a code}{the integration method---it may take more steps than allowed by the ODE integrator, require a timestep below floating point accuracy, or fail to meet the tolerances}.  The easiest way to improve the coupling is to cut the
...
terms only over $\Delta t/2$.  \added{The staggering of the reactive update
...
\citet{strang_rnaas} and showed that the above formulation \replaced{gets}{achieved} second
...
We begin with the general form of a multi-implicit SDC correction equation with two physical processes\deleted{, as originally presented in} \citep{bourlioux2003high}.
...
\item {\em Iterate}
  ...
  \item {\em Step 1: Create the advective update term, $\Adv{\Uc}^{n+1/2,(k)}$}
    ...
    \item Predict $\qb$ to the interfaces at $t^{n+1/2}$ using the CTU
      PPM method\added{---this involves taking the cell-centered
...
%%       \end{equation}
      ...
      gravitational sources, we do those in the normal predictor\added{, consistent
  ...
  %\MarginPar{Need to change this.  The point of this paper is we are NOT using method of lines (that was the older castro sdc paper).  This simplified approach is NOT method of lines.}
    ...
    usually used in reaction networks \added{(we depend on $e$ instead of
...
designed for stiff ODEs.  Our version has been ported to \replaced{C++, as a
header-only library enabling it to run on GPUs}{modern C++ and designed to run on GPUs}.  We have also added
...
simplified-SDC convergence respectively.  \added{We ran each integration method on a domain with $64^2$, $128^2$, $256^2$, $512^2$ zones.  An error is defined between successive resolutions by coarsening the higher resolution run and taking the L1 norm of the zone-by-zone difference of the two simulations---for example, $\epsilon_{64\rightarrow 128}$ is the error between the $64^2$ and $128^2$ simulations.}  \deleted{The convergence rate is
computed by taking log-base-2 of the ratio of adjacent errors.}  \added{The convergence
...
\replaced{converge nearly second order, and agree well with each
  other}{exhibit convergence at roughly second-order and agree well.}.
...
important.  \added{To highlight this, Figure~\ref{fig:strang_steps} shows the nuclear
