...
\begin{align}
\label{eq:dt:nuce}
\Delta t &\le \mathtt{dtnuc\_e}\, \min_{i,j,k} \left\{\frac{e_{i,j,k}}{\dot{e}_{i,j,k}}\right\} \\
%
\label{eq:dt:nucX}
\Delta t &\le \mathtt{dtnuc\_X}\, \min_{i,j,k} \left\{\min_n\frac{X^n_{i,j,k}}{\dot{X}^n_{i,j,k}}\right\}
\end{align}
where $\mathtt{dtnuc\_e}$ and $\mathtt{dtnuc\_X}$ are runtime parameters
used to control the allowed change.
...
\item {\em Iterate}
  ...
  \item {\em Create the advective update term, $\Adv{\Uc}^{n+1/2,(k)}$}
    ...
    \item construct the advective update terms, first ignoring the hydrodynamics sources:
      $\Advt{\Uc}^{n+1/2,(k)}_{i,j,k}$, e.g., as:
      \begin{align}
...
For this study we will use the 19 isotope ``aprox19'' network together
with a table that gives the NSE abundances in regions where the system
is in NSE.  The table provides:
\begin{align*}
...
The NSE table was generated using a 125 nuclei reaction network and is
the same as described in \cite{ma:2013}.  The compositional quantities
...
is as follows:
\begin{itemize}
\item initialize the problem, including $X_k$

\item fill the initial aux data with $Y_e$, $\bar{A}$, and $(B/A)$.

\item in hydro, update these quantities simply via advection

\item for the reactive update:

  \begin{itemize}
  \item check if NSE applies (see below)

  \item if we are in an NSE region:

    \begin{itemize}
    \item use $\rho$, $T$, and $Y_e$ to call the table. This
      returns: $dY_e/dt$, $(B/A)_{\rm out}$, and $\bar{A}_{\rm out}$.
  ...
  \item update the mass fractions, $X_k$, using the values from the table
    \end{itemize}

  \item if we are not in NSE:
...
\begin{itemize}
\item $\rho > \rho_\mathrm{nse}$
\item $T > T_\mathrm{nse}$
\item $X(\isotm{C}{12}) < A_\mathrm{nse}$
\item $X(\isotm{He}{4}) + X(\isotm{Cr}{48}) + X(\isotm{Fe}{52}) + X(\isotm{Fe}{54}) + X(\isotm{Ni}{56}) > B_\mathrm{nse}$
\end{itemize}
