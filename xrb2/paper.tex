...
\documentclass[preprint,times,tighten,linenumbers,trackchanges]{aastex63}
...
\end{equation}
Here, $\mathcal{D} = k_{th} / (\rho c_v)$ is the thermal diffusivity, $\enucdot$ is the specific
nuclear energy generation rate, and $E$ is the specific total energy \citep{timmeswoosley:1992}. 
\citet{cavecchi:2013} explored the effects of rotation on the flame propagation, and noted that the 
...
rate. These results supplemented the findings of \citet{cavecchi:2013}. Despite this formulation of 
...
in a number of ways. Firstly, the model begins without any burning taking place, and initial model assumes that a 
...
the rotation rate could be tested using observations, as it infers that the burst 
...
here is available in the git repo as {\tt
  flame\_wave}.  \added{We thank the anonymous referee for the helpful suggestions.}
