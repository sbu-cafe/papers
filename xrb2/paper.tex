...
\ref{fig:time_series_enuc_1000}, we use the energy generation rate $\dot{e}_\mathrm{nuc}$ measured at $50~\mathrm{ms}$ and $100~\mathrm{ms}$ to discern the horizontal extent of the flame at different rotation rates. Taking the edge at greatest radius of the bright teal/green region (where significant energy generation is occuring) as the leading edge of the flame in each plot, we see that the horizontal extent of the $1000~\mathrm{Hz}$ flame ($\tt{F1000}$) appears to be 
...
Figure~\ref{fig:flame_speeds_1} gives the position of the flame front as a function of time for the $\tt{F500}$ and $\tt{F1000}$ runs 
(blue and orange, respectively) to show the dependence on rotation rate. {\color{blue}In 
\citet{flame_wave1}, we 
applied a linear least-squares fit to the flame front position as a function of time to estimate the propagation velocity. 
For this study we instead fit a quadratic curve to each set of data, as some of the hotter $500~\mathrm{Hz}$ 
runs exhibit significant acceleration. We discarded the data points with $t \lesssim 20~\mathrm{ms}$, 
since these correspond to the initial transient period before the flame has begun to propagate 
steadily.} The fit parameters yield estimates of the flame acceleration and velocity at $t = 0~\mathrm{ms}$, 
...
at $t = 0~\mathrm{ms}$. We use the fit parameters to calculate the flame speeds at $t = 50~\mathrm{ms}$ (when the flame has reached steady propagation), given in the fourth column of Table \ref{table:flame_speeds}.} \MarginPar{KE: Outside of updating the data in the table, is there anything else we 
