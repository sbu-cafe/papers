...
\affiliation{Universidad Nacional Aut\'{o}noma M\'{e}xico, Instituto de Astronom\'{i}a, Ciudad Universitaria, CDMX 04510, Mexico}
...
%======================================================================
\begin{abstract}
We continue to investigate laterally propagating flames in type I X-ray bursts using
...
exception of IGR J17480-2446 (\citet{altamirano2010discovery}, spinning at $11~\mathrm{Hz}$), most observations of
...
{\color{blue}
For some of our analysis, we would like to have a means of estimating the temperature ($T$) and nuclear 
energy generation rate ($\enucdot$) in the burning region of each simulation. For this purpose, we define 
...
the subset of our runs that achieve a propagating flame. This figure is referenced throughout the 
subsequent sections.
}
...
model used here. {\color{blue} Given that the size of our domain is $\sim 1~L_R$ for {\tt F250} (using Equation~\ref{eqn:rossby}), it is also possible that we simply cannot confine the flame sufficiently with our current domain width.}

\MarginPar{NF: Each of the figures referenced in this paragraph contributes to the idea that we are seeing burning/energy generation from fluid undergoing vortical motion/fuel being entrained around the flame edge... I just think the logical flow of the sentences could be a bit better.}
Burning in the {\tt F500} and {\tt F1000} runs is concentrated in a hot, dense region with circular 
motion. {\color{blue} We visualize the velocity field in the burning region for the {\tt F500} run in Figure~\ref{fig:streamlines}, colored by temperature. Velocities near the bottom edge of the high temperature/high density region (white) tend to have a significant $+z$ component. This boundary most likely corresponds to the leading edge of the flame, where colder fuel undergoes vortical motion and becomes entrained into the burning front.} In Figure~\ref{fig:urho}, which compares the horizontal $r$-velocity $u$, density $\rho$ and the 
...
vortical motion, shown in the Figure~\ref{fig:uv} phase plots comparing $u$, vertical z-velocity $v$ and $\dot{e}_{\rm 
nuc}$. {\color{blue} This agrees with the velocity streamlines shown in Figure~\ref{fig:streamlines}.}
This feature is not developed in the $250~\mathrm{Hz}$ flame in Figure~\ref{fig:uv} {\color{blue} (left panel)}; it could potentially develop 
...
\begin{figure}
  \plotone{streamlines_500Hz_temp.pdf}
  \caption{\label{fig:streamlines}Streamlines showing the velocity field at time $t = 
  100~\mathrm{ms}$ for the {\tt F500} run. Streamline segments are colored by transverse velocity 
  (yellower implies faster flow), and segments with transverse velocity $v < 2 \times 
  10^6~\mathrm{cm/s}$ are scrubbed out. Note that the plot has been stretched vertically to 
  better show the streamlines -- the aspect ratio is not the aspect ratio of the simulation 
  domain.}
\end{figure}
...
{\color{blue} (see Figure~\ref{fig:maxima}, right panel)}, even at the latest timesteps ($\sim 
70~\mathrm{ms}$, when the flame is most developed). {\color{blue} Significant $\enucdot$ values for 
...
\ref{fig:compare_500Hz_abar}, \ref{fig:time_series_500}, and \ref{fig:time_series_1000}). 
...
$T_{\mathrm{star}}$ is increased, the flame becomes greatly accelerated. The initial flame 
...
40~\mathrm{ms}$. The reason for the acceleration of the flame is not entirely clear. It could be 
...
We ran a number of simulations of laterally propagating flames in XRBs in order to explore the effects of rotation and thermal structure. We found that increasing the rotation rate increased the energy generation rate within the flame and enhanced nuclear burning. {\color{blue} Apart from the lowest rotation run (which failed to ignite), flame propagation was not noticeably impacted by rotation rate; by the time the different flame fronts reached steady propagation, they shared comparable velocities. These results are likely due to the rotation-dependent strength of the Coriolis force and confinement of the flame.} \MarginPar{NF: is the wording of the blue text correct?} We explored several models with different crustal temperatures, to determine what effects mechanisms such as higher accretion rates, crustal heating and shallow heating may have on flame propagation. We found that increasing the temperature of the crust significantly enhanced the flame propagation. This we believe to be because a cooler crust allows heat to more efficiently be transferred from the flame itself, therefore reducing the flame's temperature, slowing burning and consequently reducing its propagation speed. We saw that if the crust temperature was too high, then instead of a flame, the atmosphere would burn stably. This is reminiscent of what is seen for neutron stars with accretion rates exceeding the Eddington luminosity.
