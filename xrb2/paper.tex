...
\begin{figure}[t]
	\centering
	\plotone{flame_speeds_rotation.pdf} % Not certain what style we want for the plot here
	\caption{\label{fig:flame_speeds_1} Plots of flame front position vs.\ time for the 500 Hz and 1000 Hz runs. The dashed lines
...
\begin{figure}[t]
	...
	\caption{\label{fig:flame_speeds_2} Plots of flame front position vs.\ time for the three 500 Hz runs with
		$T_{star} \leq 3 \times 10^8~\mathrm{K}$. The dashed lines show quadratic least-squares fits to the data
		for $t \gtrsim 20~\mathrm{ms}$.}
\end{figure}
...
parameters. These results are given in Table~\ref{table:flame_speeds}. We also include the flame speed at $t = 100~\mathrm{ms}$ extrapolated
from the fit.
...
Figure~\ref{fig:flame_speeds_1}, there is no obvious inverse scaling with rotation rate from this set of runs. We observed earlier,
...
small deceleration at early times. This disparity could be due to the 500 Hz flame taking more time to develop
fully and reach higher burning rates, or a result of the difference in Coriolis force. \MarginPar{Do we have other ideas why?}
...
feel free to move it. What's in this section now is just an initial draft/outline.}

\begin{deluxetable}{lrrr}
	\tablecaption{\label{table:flame_speeds} Flame speed and acceleration values measured for each simulation. $s_0$ is the speed at $t = 0~\mathrm{ms}$, and $s_{100}$ is the speed at $t = 100~\mathrm{ms}$.}
	\tablehead{\colhead{run} & \colhead{$s_0$ (km s$^{-1}$)} & \colhead{$s_{100}$ (km s$^{-1}$)} & \colhead{$a$ (km s$^{-2}$)}} 
	\startdata
	{\tt flame\_wave\_500}      & $3.08 \pm 0.01$ & $3.62 \pm 0.02$ & $5.41 \pm 0.19$ \\
	{\tt flame\_wave\_1000}     & $3.41 \pm 0.01$ & $3.31 \pm 0.02$ & $-1.03 \pm 0.13$ \\
	{\tt flame\_wave\_500\_2E8} & $3.76 \pm 0.02$ & $\sim 3.93$ & $1.74 \pm 0.17$ \\
	{\tt flame\_wave\_500\_3E8} & $5.29 \pm 0.10$ & $\sim 41.0$ & $357.4 \pm 1.5$ \\
	\enddata
	%
\end{deluxetable}
