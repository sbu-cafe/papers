...
Figure~\ref{fig:flame_speeds_1} gives position vs.\ time for the $\tt{flame\_wave\_500}$ and $\tt{flame\_wave\_1000}$ runs to show the dependence on rotation rate. As some of the hotter 500~Hz runs exhibit significant accelerations, we fit a quadratic curve to each set of data and calculate the acceleration and initial velocity from the fit parameters. These results are given in Table~\ref{table:flame_speeds}.
...
{\color{red} On the subject of things breaking out of the atmosphere, discuss interesting characteristics of the extreme 4E8 run here: maybe include a plot showing abar and temp at 40 ms, and another plot showing the momentum components?}
...
{\color{red} Discuss flame acceleration here, it's sort of mysterious but could be due to energy released from burning dominating the flame propagation and increasing speed over time, or maybe enhanced mixing/diffusion is pulling in more fuel to burn, or maybe the average fuel layer temp is going up, making it easier for the flame to burn and propagate}
