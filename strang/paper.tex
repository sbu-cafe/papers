...
The equations of hydrodynamics with reacting sources are:
\begin{align}
...
When we are reacting, we need the temperature, so we can look at a temperature evolution
equation:
\begin{equation}
...
equilibrium).  During reactions, we neglect the hydrodynamics terms,
...
\item Evolve the temperature equation, but ``freeze'' the specific
  ...
  \citet{ABNZ:III} and is the default method in \castro.
