...
and \citet{Cavecchi2019} tractable. The first was the assumption of vertical hydrostatic equilibrium,
which allowed them to filter out vertically propagating sound waves that would otherwise
have limited the timestep. This enabled them to reach the simulation times (on the
order of seconds) needed to capture the rise of the burst, but they could not model
vortical fluid motions driven by effects such as turbulence and convection. These can
facilitate mixing and help to accelerate the propagation of the flame front. They also
included only the triple-$\alpha$ process in their implementation of nuclear burning,
and use a fixed, constant opacity across the entire domain for each simulation run (although
they explore the dependence of the flame propagation speed on this opacity value).
...
disallowed in the $450$ Hz runs by the enforcement of vertical hydrostatic equilibrium (we already saw
