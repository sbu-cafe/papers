...
the highly-stratified burning layer in an X-ray burst.  Simple \MarginPar{is simple the right word here?} \MarginPar{It might be better to say simulations are performed through a simplified atmosphere?}
...
time scales at play in an X-ray burst.  Our studies complement 
...
atmosphere's structure we not captured.  Their model showed that the
...
as the nature of any turbulence the burning front might encounter as it propagates through the atmosphere.
...
force creates a geometrical configurations that increases the flame
...
effect on flame propagation of changing Coriolis confinement across
the surface was explored in \citet{art-2015-cavecchi-etal}, while \citet{art-2016-cavecchi-etal}
showed how the magnetic field tension, opposing the Coriolis
force, can either speed up or slow down the flame by changing the
flame front orizontal extent. Finally, \citet{Cavecchi2019}
...
\end{eqnarray}
Here, $\rho$ is the mass density, $\Ub$ is the velocity, $p$ is the pressure and $E$ is
...
explicitly in time. \MarginPar{Have not defined what r is in eqs 1-3, and Omega and g are only defined with respect to eq. 5}
...
13-isotope alpha chain, derived from the {\tt aprox13}
...
Flame propagation rates from both the 1D laminar flames and full 2D simulations are given in Table \ref{table:flame_speeds}.  \MarginPar{I think we should only report the laminar speeds here and defer the turbulent speeds until the section where we talk about how they are computed}

\begin{deluxetable}{lc}
	\tablecaption{\label{table:flame_speeds} Laminar flame speeds.}
...
Table~\ref{table:sim_names} summarizes the simulations that we ran. \MarginPar{This table has different model names to the previous table}
...
atmosphere where we are most dense, with an artificial change in the
...
of the initial model using the fine grid resolution for HSE. \MarginPar{Remind the reader what this
  ...
  drop} As we'll see when the lower resolution case, we do not believe
that this affects the results.
...
which will be useful when we transition to 3-d simulations.
...
location beyond the hottest part of the flame\MarginPar{what does this mean?} where
$\dot{e}_\mathrm{nuc}$ first drops to less than $0.1 \%$ of its
maximum value. Using $0.1 \%$ of the peak value rather than the peak value
...
burning region. We also average $\dot{e}_\mathrm{nuc}$ over the  \MarginPar{I think we should discuss the averaging first, before we discuss the peak value}
...
\begin{deluxetable}{lc}
	\tablecaption{\label{table:flame_speeds_multid} Flame speeds measured from 2D calculations.}
	\tablehead{\colhead{run} & \colhead{$s_\mathrm{front}$ (km s$^{-1}$)}} 
	\startdata
	10/10 boost & $9.18 \pm 0.03$ \\
	5/5 boost & $4.00 \pm 0.01$ \\
...
Table \ref{table:flame_speeds_multid} gives the flame speed measured in each
multidimensional simulation, as well as laminar speeds obtained from
corresponding 1D runs. The 2D flames propagate at speeds about an
order of magnitude faster than their 1D counterparts. The increase in
flame speed is likely a product of the larger flame surface area and hydrodynamical effects such as
...
As we see in Figure~\ref{fig:flame_speed}, all of the various
approximations behave as we would expect.  The 1000~Hz run goes 2
times faster than the 2000~Hz run, as expected by the reduced rossby
radius \citep{cavecchi:2013}).  The 5/5 boost run is half the speed of
the 10/10 boost run, also as expected, consistent with the 1D laminar
flames and behaving as predicted by Eq.~\ref{eq:flame_scaling}.  This
confirms that the role of the Coriolis force is really to boost the
laminar flame by the geometrical/hydrodynamical effect expected ($v_f
= v_l * L_r / H$), similarly to what the 1000 Hz simulation
said.   
...
{\color{red} Show whatever it was that Don cooked up. Also need to discuss the streamline plot.}
...
\citep{Malone2014a}. As we are considering a 2d \MarginPar{We've used
  at least three different ways of writing 2-d/2D/2d so far. These
  should be changed so we used one style throughout.}system, the
...
of the flame (see also \citet{cavecchi:2013}). This same vorticity,
in 3D, should perturb the flame front and further increase flame speed
\citep{Cavecchi2019}.
...
with a constant diffusion coefficent.  The point of this test is to
demonstrate that the implementation done in a predictor-corrector
form is second-order accurate.

We use a 2-d axisymmetric geometry, with a reflection boundary on the symmetry axis (left)
...
\begin{deluxetable}{rL}
	...
	\tablehead{\colhead{base resolution} & \colhead{L-inf error}}
	\startdata
...
error.\MarginPar{This would be clearer if there was a third column showing the ratios of the errors?}  Figure~\ref{fig:diffusion} shows the temperature at the end of
