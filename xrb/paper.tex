...
rate, $\epsdot$\MarginPar{Is notation consistent between E, e, epsilondot, and enucdot?}.  The total mass conservation,
\begin{equation}
...
$w$-velocity equation as a simple advection equation:\MarginPar{In conjunction with total mass conservation?}
\begin{equation}
...
Table~\ref{table:sim_names} summarizes the simulations that we ran. \MarginPar{This table has different model names from the flame speed table}
...
of the initial model using the fine grid resolution for HSE. \MarginPar{Remind the reader what this
  ...
  drop} As we will see in the lower resolution case, we do not believe
...
with the standard 13-isotope ``aprox13'' network and the reduced\MarginPar{We use {\tt tt} instead of quotes for the networks in the numerical approach section.}
...
corresponding 1D runs. {\color{red} Should we include the laminar speeds again for reference?}
The 2D flames propagate at speeds about an order of magnitude faster than their 1D counterparts. 
The increase in flame speed is likely a product of the larger flame surface area and
hydrodynamical effects such as turbulence, wrinkling, and convective cycles, which
bring cooler fuel from ahead of the front into the hottest part of the burning region.
...
of the flame (see also \citet{cavecchi:2013}). In 3D, his same vorticity
