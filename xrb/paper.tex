...
\cite{cavecchi:2013,art-2015-cavecchi-etal,art-2016-cavecchi-etal,Cavecchi2017,Cavecchi2019} \MarginPar{need a summary}
...
\begin{figure}[t]
	\plotone{grids.pdf}
	\caption{\label{fig:grids} Slice through a simulation showing the four-level
		grid structure. Note that the boxes shown are not the simulation zones, which
		span $10~\mathrm{cm}$ at the finest level, but subdomains containing
		approximately equal numbers of zones that are each managed by a separate
		MPI process. A coarse base grid extending to the upper boundary is drawn
		in white, with the green, blue, and black grids showing the jumps in
		refinement needed to fully resolve the fuel layer and underlying neutron star.}
\end{figure}
...
\caption{\label{fig:flame} Time-evolution of the 10$\times$ boosted 1D
	...
	generation profiles at 11 different time points, while the right plot
...
$\rho_i$ using a Newton solve together with the equation of state, with either the temperature
...
\citep{Malone2014a}. As we are considering a 2d \MarginPar{We've used at least three different ways of writing 2-d/2D/2d so far. These should be changed so we used one style throughout.}system, the component of the
...
To further demonstrate the relationships between different properties of the flame, in Figure~\ref{fig:phase_plots} we present some phase plots of the energy generation rate. The phase plot of the
energy generation rate as a function of
the $x$- and $y$-velocities shows that energy is preferentially generated in
regions with negative $x$-velocity. This most likely corresponds to the burning along the underside of the flame front, where fuel has been entrained along the surface of the flame and so is moving in the opposite direction to the direction the flame is propagating in. This is further supported by the phase plot of the energy generation rate as a function of the $x$-velocity
