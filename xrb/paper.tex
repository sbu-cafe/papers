...
observations of XRBs have provided evidence that the burning begins in
a localized region and spreads over the surface of the neutron
star~\citep{bhattacharyya:2006,bhattacharyya:2007}.
...
observables to accretion and reaction rates~\citep{Jose2010a,Lampe2016}\MarginPar{ref}.
...
is the reactant)~\citep{Weinberg2006b,Weinberg2007}. \MarginPar{ref}
...
shallow-water approximation, so the vertical details of the atmosphere's
structure we not modeled.
...
structure is thin compared to the size of the system (see, e.g., \citet{Ropke2007}\MarginPar{refs} for applications to Type Ia supernovae).  For XRBs however, the flame thickness is comparable to the scale
height of the atmosphere, so we cannot use these approximations.
...
simulations.  \MarginPar{true for all?} The base grid for our standard
...
The speed and thickness of a laminar helium flame is determined by the
energy generation rate and conductivity and scale roughly as \MarginPar{need a ref}
\begin{equation}
...
microphysics.  Figure~\ref{fig:flame} shows the flame properties for a \AssignTo{Kiran and Mike will remake with the 10/10 boosted flame}
He flame using the standard conductivities and our simple reaction
network.  This flame had a density of $2\times 10^6~\gcc$ and \MarginPar{double check}
...
\begin{figure*}[t]
\plottwo{flame}{speed}
\caption{\label{fig:flame} Our ``standard'' flame. {\color{red} this should be redone for the boosted flame}}
\end{figure*}
...
is less than half our domain width.  Table~\ref{table:sim_names} lists
the simulations run and their parameters.
...
\tablecaption{\label{table:sim_names} Simulaiton parameters.}
\tablehead{\colhead{name} & \colhead{reaction boost} & \colhead{conductivity boost} & \colhead{fine grid resolution} & \colhead{rotation rate}  & \colhead{network}}
\startdata
10/10 & 10 & 10 & 10~cm & 2000~Hz & {\tt aprox13} \\
10/10-slow & 10 & 10 & 10~cm & 1000~Hz & {\tt aprox13} \\
10/10-lores & 10 & 10 & 20~cm & 2000~Hz & {\tt aprox13} \\
5/5 & 5 & 5 & 10~cm & 2000~Hz & {\tt aprox13} \\
10/10-iso7 & 10 & 10 & 10~cm & 2000~Hz & {\tt iso7} \\
\enddata
\end{deluxetable}
...
\AssignTo{Mike, Kiran, Alice}

Figure~\ref{fig:time_series} shows the time 

Here we'll show the large scale dynamics, colormap plots, etc. and discuss the trends.
...
\begin{figure}[h]
\centering
\plotone{flame_boost_10_10_overview}
\caption{\label{fig:10_10_overview} Temperature, energy generation rate, and out-of-plane velocity for the 10/10 simulation.}
\end{figure}
...
To measure the propagation rate of the burning front, we track the location beyond the hottest part of the flame where $\dot{e}_\mathrm{nuc}$ first drops to less than $0.1 \%$ of maximum. Using $0.1 \%$ of the peak value rather than the peak value itself helps to mitigate fluctuations caused by localized fluid motions, as the reduced value lies in a less turbulent part of the burning region. We also average $\dot{e}_\mathrm{nuc}$ over the vertical coordinate to further dampen these fluctuations. After an initial transient period typically spanning $\sim 3$ ms, the flame settles into a state of steady propagation. The position data here are well fitted by a linear function of time (as seen in Figure \ref{fig:flame_speed}), and the resulting slope gives the velocity of the flame front.

Table \ref{table:flame_speeds} gives the flame speed measured in each multidimensional simulation, as well as laminar speeds obtained from any corresponding 1D runs. The 2D flames propagate at speeds about an order of magnitude faster than their 1D counterparts. The increase in flame speed is likely a product of hydrodynamical effects such as turbulence, wrinkling, and convective cycles, which bring cooler fuel from ahead of the front into the hottest part of the burning region.
...
\begin{deluxetable}{lcc}
	...
	\startdata
	10/10 boost & $9.46 \pm 0.04$  & $1.06 \pm 0.01$ \\
	5/5 boost & $3.97 \pm 0.01$ & $0.56 \pm 0.01$ \\
	iso7 network & $7.66 \pm 0.04$ & $-$ \\
	1000 Hz & $26.1 \pm 0.20$ & $-$ \\
	\enddata
	%
\end{deluxetable}
...
\begin{figure}[t]
\centering
\plotone{plots/results/speed_10_10.pdf}
\caption{\label{fig:flame_speed}The position of the burning front for the 10/10 flame as a function of time. The dashed line shows a linear least squares fit for $t \gtrsim 10$ ms.}
\end{figure}
...
$\ln \left(\mathbf{\psi}\right)$ at time $t = 0.02~\mathrm{s}$.}
\end{figure}
...
We also try a rotation period of $P_\mathrm{rot} \sim 10^{-3}~\mathrm{s}$.

\subsection{Reaction Network}

\AssignTo{Mike, Kiran}

We look at iso7 instead of aprox13



\subsection{Resolution}

\MarginPar{do we want to do a low res run?}
...
model these XRBs.  First, as shown in \citet{castro_sdc}, we have
