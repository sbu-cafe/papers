...
Flame propagation rates from both the 1D laminar flames and full 2D simulations are
given in Table \ref{table:flame_speeds}. 
...
temperature from the EOS.  Because of the strong degeneracy at the
high densities at the bottom of the domain, the numerical error in the
interpolation can create a temperature jump there.\MarginPar{discuss better}
...
temperature at the refinement boundary due to the interpolation
procedure of our initial model which focused on pressure and density
rather than temperature. \MarginPar{Remind the reader what this
...
\begin{equation}
  T(r) = T_1 + (T_2 - T_1) \left (\frac{t_0}{t + t_0} \right )^{3/2} e^{-r^2/(4 D (t + t_0))}
\end{equation}
