...
\caption{\label{fig:castro-scaling} (left) \castro\ strong scaling on OLCF titan
for a pure hydro (Sedov) problem; (right) \castro\ strong scaling
on OLCF titan for a hydro + Poisson gravity (wdmerger) problem.}
\end{figure}
...
\caption{\label{fig:knl-gpu-scaling} \castro\ scaling on new architectures.}
\end{figure}
...
Figure~\ref{fig:knl-gpu-scaling} shows the performance of \starlord\ on the
